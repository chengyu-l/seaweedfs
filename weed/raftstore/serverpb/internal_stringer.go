...
func (as *InternalRaftStringer) String() string {
	switch {
	case as.Request.ClientRequest != nil:
		switch as.Request.ClientRequest.CmdType {
		case CmdType_Put:
			return fmt.Sprintf("ID:<%d> put:<%s>",
				as.Request.ID,
				NewLoggablePutRequest(as.Request.ClientRequest.Put).String(),
			)
		case CmdType_Txn:
			return fmt.Sprintf("ID:<%d> txn:<%s>",
				as.Request.ID,
				NewLoggableTxnRequest(as.Request.ClientRequest.Txn).String(),
			)
		}
	default:
		// nothing to redact
