...
const (
	...
	msgAppBufSize = 1024 * 1024
)

// msgapp stream sends three types of message: linkHeartbeatMessage,
...
func newMsgAppEncoder(w io.Writer) *msgAppEncoder {
	return &msgAppEncoder{
		w:         w,
		buf:       make([]byte, msgAppBufSize),
...
func (enc *msgAppEncoder) encode(m *raftpb.Message) error {
	...
	case enc.index == m.Index && enc.term == m.LogTerm && m.LogTerm == m.Term:
		...
		for i := 0; i < len(m.Entries); i++ {
			...
			if n := m.Entries[i].Size(); n < msgAppBufSize {
...
func newMsgAppDecoder(r io.Reader, local, remote types.ID) *msgAppDecoder {
	return &msgAppDecoder{
		r:         r,
		local:     local,
		remote:    remote,
		buf:       make([]byte, msgAppBufSize),
