...
func (h *pipelineHandler) ServeHTTP(w http.ResponseWriter, r *http.Request) {
	...
	w.Header().Set("X-Nebulasfs-Cluster-ID", h.cid.String())
...
type snapshotHandler struct {
	...
	tr          raftnode.Transporter
	r           Raft
	snapshotMgr raftnode.SnapshotMgr

	localID types.ID
	cid     types.ID
}

func newSnapshotHandler(t *Transport, r Raft, snapshotMgr raftnode.SnapshotMgr, cid types.ID) http.Handler {
...
func (h *snapshotHandler) ServeHTTP(w http.ResponseWriter, r *http.Request) {
	...
	n, err := h.snapshotMgr.SaveDBFrom(r.Body, m.Snapshot.Metadata.Index)
	if err != nil {
...
func (h *streamHandler) ServeHTTP(w http.ResponseWriter, r *http.Request) {
	...
	case streamTypeMsgApp.endpoint():
		t = streamTypeMsgApp
	...
	default:
		if h.lg != nil {
			...
			glog.Infof("ignored unexpected streaming request path %s", r.URL.Path)
		}
	...
	if err != nil {
		if h.lg != nil {
			...
			glog.Errorf("failed to parse from %s into ID (%v)", fromStr, err)
		}
	...
	if h.r.IsIDRemoved(uint64(from)) {
		if h.lg != nil {
			...
			glog.Warningf("rejected the stream from peer %s since it was removed", from)
		}
	...
	if p == nil {
		...
		// 2. local falls behind of the cluster, and cannot recognize
		...
		if h.lg != nil {
			...
			glog.Errorf("failed to find member %s in cluster %s", from, h.cid)
		}
	...
	if gto := r.Header.Get("X-Raft-To"); gto != wto {
		if h.lg != nil {
			...
			glog.Errorf("streaming request ignored (ID mismatch got %s want %s)", gto, wto)
		}
...
func checkClusterCompatibilityFromHeader(lg *zap.Logger, localID types.ID, header http.Header, cid types.ID) error {
	...
	if gcid := header.Get("X-Nebulasfs-Cluster-ID"); gcid != cid.String() {
		lg.Warn(
