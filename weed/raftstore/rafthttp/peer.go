...
type Peer interface {
	...
	// is similar to send.
	sendSnap(m raftnode.SnapMessage)
...
type peer struct {
	...
	msgAppWriter *streamWriter // MsgApp speicial
	writer       *streamWriter // All msgs
	pipeline     *pipeline
...
func startPeer(t *Transport, urls types.URLs, peerID types.ID) *peer {
	...
	p := &peer{
		...
		status:       status,
		picker:       picker,
		msgAppWriter: startStreamWriter(t.Logger, t.ID, peerID, status, r),
			...
			case mm := <-p.recvc:
				if err := r.Process(ctx, mm); err != nil {
					if t.Logger != nil {
						...
						glog.Warningf("failed to process raft message (%v)", err)
					}
				}
			case <-p.stopc:
				return
	...
	default:
		...
		if p.status.isActive() {
			if p.lg != nil {
				...
				glog.Warningf("dropped internal raft message to %s since %s's sending buffer is full (bad/overloaded network)", p.id, name)
			}
		} else {
			if p.lg != nil {
				p.lg.Warn(
				...
				glog.Infof("dropped %s to %s since %s's sending buffer is full", m.Type, p.id, name)
			}
		}
...
func (p *peer) sendSnap(m raftnode.SnapMessage) {
...
func (p *peer) stop() {
	if p.lg != nil {
		...
		glog.Infof("stopping peer %s...", p.id)
	}
	...
	defer func() {
		if p.lg != nil {
			...
			glog.Infof("stopped peer %s", p.id)
		}
	}()
