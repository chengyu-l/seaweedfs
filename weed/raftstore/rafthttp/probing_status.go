...
func addPeerToProber(lg *zap.Logger, p probing.Prober, id string, us []string, roundTripperName string, rttSecProm *prometheus.HistogramVec) {
	...
	if err != nil {
		if lg != nil {
			...
			glog.Errorf("failed to add peer %s into prober", id)
		}
		return
	}
		...
		case <-time.After(interval):
			if !s.Health() {
				if lg != nil {
					...
					glog.Warningf("health check for peer %s could not connect: %v", id, s.Err())
				}
				interval = statusErrorInterval
			} else {
			...
			if s.ClockDiff() > time.Second {
				if lg != nil {
					...
					glog.Warningf("the clock difference against peer %s is too high [%v > %v]", id, s.ClockDiff(), time.Second)
				}
			}
