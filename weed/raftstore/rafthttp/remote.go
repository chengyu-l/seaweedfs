	...
	default:
		if g.status.isActive() {
			if g.lg != nil {
				...
				glog.Warningf("dropped internal raft message to %s since sending buffer is full (bad/overloaded network)", g.id)
			}
		} else {
			if g.lg != nil {
				g.lg.Warn(
				...
				glog.Infof("dropped %s to %s since sending buffer is full", m.Type, g.id)
			}
		}
