...
func unsafeRange(c *bolt.Cursor, key, endKey []byte, limit int64) (keys [][]byte, vs [][]byte) {
	...
	if len(endKey) > 0 {
		...
		if len(endKey) == 1 && endKey[0] == 0 {
			isMatch = func(b []byte) bool { return true }
		}
	} else {
...
func unsafeRangeMap(c *bolt.Cursor, key, endKey []byte, limit int64, deletedKeys map[string]bool, greaterThan bool) (kvs map[string][]byte) {
	if limit <= 0 {
		limit = math.MaxInt64
	}
	isDeleted := func(b []byte) bool {
		if deletedKeys == nil {
			return false
		}
		if _, deleted := deletedKeys[string(b)]; deleted {
			return true
		}
		return false
	}
	...
	if len(endKey) > 0 {
		isMatch = func(b []byte) bool {
			return !isDeleted(b) && bytes.Compare(b, endKey) < 0
		}
		if greaterThan {
			isMatch = func(b []byte) bool { return !isDeleted(b) }
		}
	} else {
		isMatch = func(b []byte) bool { return !isDeleted(b) && bytes.Equal(b, key) }
		limit = 1
	}
